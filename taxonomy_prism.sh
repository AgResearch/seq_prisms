...
function fake_prism() {
   echo "dry run ! "
   make -n -f taxonomy_prism.mk -d  --no-builtin-rules -j 16 hpc_type=$HPC_TYPE sample_rate=$SAMPLE_RATE data_dir=$DATA_DIR out_dir=$OUT_DIR  $TARGETS > $OUT_DIR/taxonomy_prism.log 2>&1
...
function run_prism() {
   make -f taxonomy_prism.mk -d  --no-builtin-rules -j 16 hpc_type=$HPC_TYPE sample_rate=$SAMPLE_RATE data_dir=$DATA_DIR out_dir=$OUT_DIR $TARGETS > $OUT_DIR/taxonomy_prism.log 2>&1
