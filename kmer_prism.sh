...
function get_targets() {
   ...
   for file in $FILES; do
      base=`basename $file`
      TARGETS="$TARGETS $OUT_DIR/${base}.kmer_prism"
   done 
}
...
function fake_prism() {
   ...
   tardis.py -q -hpctype $HPC_TYPE -d $OUT_DIR $OUT_DIR/kmer_entropy.py -s .00015 -t zipfian -k 8 -p 30 -o zipfian8.txt -b /dataset/Tash_FL1_Ryegrass/ztmp/seq_qc/kmer_analysis /dataset/Tash_FL1_Ryegrass/ztmp/For_Alan/*.fastq.gz
...
function run_prism() {
   ...
   make -f kmer_prism.mk -d -k  --no-builtin-rules -j 16 hpc_type=$HPC_TYPE sample_rate=$SAMPLE_RATE data_dir=$DATA_DIR out_dir=$OUT_DIR $TARGETS > $OUT_DIR/kmer_prism.log 2>&1
   # this uses the pickled distributions to make the final spectra
   tardis.py -hpctype $HPC_TYPE -d $OUT_DIR kmer_entropy.py -k 6 -t zipfian -o $OUT_DIR/kmer_summay.txt -b $OUT_DIR $TARGETS
   ...
   tardis.py -hpctype $HPC_TYPE -d $OUT_DIR Rscript --vanilla kmer_plots_gbs.r datafolder=$OUT_DIR
}
