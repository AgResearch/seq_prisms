...
draw_heatmap <- function(datamatrix, num_clust) {
   # want to plot num_clust  broad locusonomy profiles - so cluster the profiles (if necessary)
  ...
  dev.off()
   ...
   jpeg(filename = paste(moniker, ".jpg",sep=""), width=1400, height=2000) # with dendrograms
   hm<-heatmap.2(as.matrix(clustered_data),  scale = "none", 
       dendrogram = "col",  
       trace="none",
       #trace = "none", breaks =  -2 + 4/9*seq(0,11), 
   ...
   write.table(rownames(as.matrix(clustered_data))[hm$rowInd[length(hm$rowInd):1]] , file=paste(moniker, "_locusnames_ordered.dat",sep=""),row.names=TRUE,sep="\t")

   if ( ! is.na( clustering ) ) {
      # supply the locus clusters
      write.table(clustering$cluster, file=paste(moniker, "_locus_clusters.dat",sep=""),row.names=TRUE,sep="\t")
      # supply the names given to the locus clusters
      write.table(rownames, file=paste(moniker, "_locus_cluster_names.dat",sep=""),row.names=TRUE,sep="\t")
   }
...
draw_abundant_heatmap <- function(datamatrix) {
   ...
   # want to plot the 40 most abdundant locusa (no clustering).
  ...
  dev.off()
   ...
   hm<-heatmap.2(as.matrix(sdatamatrix),  scale = "none", 
       ...
       keysize=1.0, margin=c(27,78), cexRow=1.3, cexCol=1.3, 
       lmat=rbind(  c(4,3,0 ), c(2, 1, 0) ), lwid=c(.1, 1.2, 0 ), lhei=c(.25, 3),labCol=colLabels)
   title(main="40 most abundant locus")
   ...
   # the row labels on the plots may be truncated  so supply a file with the
   # row  names ordered as per the plot
   write.table(rownames(as.matrix(sdatamatrix))[hm$rowInd[length(hm$rowInd):1]] , file=paste(moniker, "_abundant_locusnames_ordered.dat",sep=""),row.names=TRUE,sep="\t")
